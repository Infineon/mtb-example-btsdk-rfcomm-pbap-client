...
## Overview
This app performs as a Bluetooth&#174; PBAP Client. It can connect to a mobile phone that supports PBAP Server profile and download phone book and call logs.
...
2. From the phone, initiate device discovery and find the pbap\_client application and perform pairing.
...
5. Run the BTSpy app, capture protocol and snoop traces, and view the traces via a viewer such as Frontline.

Note: only the first 100 records will be displayed. The application can be updated to get more records if desired.

Note for iPhone users: Upon pairing, the iPhone will show an error in establishing the connection. This can be ignored. After establishing the connection from the Client Control UI, go to Settings->Bluetooth UI on the iPhone, select the paired device (pbap client), and enable UI to 'Sync Contacts' before attempting to use the Client Control buttons for accessing contacts or call logs.
