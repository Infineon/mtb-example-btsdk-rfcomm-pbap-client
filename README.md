...
Note for iPhone users: Upon pairing, the iPhone will show an error in establishing the connection. This can be ignored. After establishing the connection from the Client Control UI, go to Settings->Blueooth UI on the iPhone, select the paired device (pbap client), and enable UI to 'Sync Contacts' before attempting to use the Client Control buttons for accessing contacts or call logs.
